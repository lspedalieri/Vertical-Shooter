...
func tween_scale(original_scale = null) -> void:
	...
	if original_scale == null:
		...
		print(sprite.scale)
		tween.tween_property(sprite, "scale", original_scale, scale_duration * 0.1).from_current()
		tween.tween_property(sprite, "scale", Vector2.ONE, scale_duration * 0.9).from(original_scale)
